...
class KboProvider:
    ...
    def _parse_schedule_page(self) -> list[GameSchedule]:
        ...
        for row_data in payload.get("rows", []):
            ...
            date_label = self._clean_text(row[0].get("Text", "")) or "오늘 경기"
            time = self._clean_text(row[1].get("Text", ""))
            away_team, home_team = self._parse_matchup_html(row[2].get("Text", ""))
        ...
        if upcoming:
            return upcoming[:10]
        return [game for _, game in games[:10]]
